#!/usr/bin/env node
// QVAC Fake Spam Email Subject Line Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29388;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/spamsubject") {
      try {
        ...
        const result = await generate(modelId, topic.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Spam Email Subject Line Generator GUI ready at http://localhost:${PORT}`);
  });
